...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private void pyramid(float size, float n, GL2 gl2) { // Draws a cube with side length = size
        gl2.glPushMatrix(); // Save a copy of the current matrix.
        gl2.glScalef(size, size, size); // Scale unit cube to desired size

        gl2.glPushMatrix();
        gl2.glRotatef(90, 1, 0, 0); // Rotates it so base is at the bottom
        gl2.glTranslatef(0, 0, -1); // Translated it to the vertical center
        for (int i = 0; i < n; i++) {
            triangle(i - 1, i, n, gl2); // Calls the triangle() method with parameters form the loop
        }
        gl2.glPopMatrix();
    ...
    private void spiral(double n, GL2 gl2) {
        double pi = Math.PI; // For covenience
        double t = 5; // Height of spiral
        double radius = 3;
        double angle, nextAngle, x, y, z, h; // angle and orgAngle are angles that makes use in while() loop, h is height between each "layer"

        h = t / 1550; // t / x, where x is how accurate spiral will be
        y = -5; // Translates spiral to the center
        ...
        while (y <= t) {
            ...
            y = y + h;
            angle += (n / 9); // makes the input parameter n work
        }
        gl2.glEnd();
    }
